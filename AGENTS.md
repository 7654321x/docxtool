...
批量测试必须在 `test_docx/end_docx` 生成 JSON 和文字报告，记录总数、成功/失败数、各级问题数、模板匹配、段落和标题统计、版头/落款/附件识别、文字新增/丢失、表格图片、空白页、处理耗时和错误信息。报告不得包含完整正文、密钥、Cookie 或完整日志。发布前或用户要求“全部测试”时，使用 `scripts/batch_test_docx.py --render-review --require-render` 抽查标准集 10 篇和专项集全部文档；渲染图、PDF 与疑似空白页清单写入对应结果目录。若没有可用渲染器，必须明确记录“未执行视觉渲染检查”，不能据此断言没有空白页。
