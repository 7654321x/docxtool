...
当前正式版本为 **5.3**。本版本确立 Recognition 最终语义类型、规范化后 Final DocumentStructure 和统一 LayoutPolicy 的职责边界；附件人工列布局可在统一字体字号的同时保留 Tab、连续空格、全角空格和 NBSP，结构段落渲染失败会明确中止，避免伪装成正文成功输出。继续保持识别优先 wheel 供 WPS、Office.js、VSTO 和其他本地宿主复用。
...
- 可选 `letterhead` 配置用于自动识别或在首页正文流重建单机关标志、结构化发文字号、上行文签发人和直线/五角星分割线；机关标志按版心自适应，红线至标题使用正文行距 × 2。当前界面只启用单机关发文。
...
WPS 任务窗格的“添加版头”可填写发文机关标志、发文字号、可选签发人并选择直线型或五角星型分割线，随后通过本机文档事务直接生成到当前文档，不触发全文识别、一键排版或公网授权。已有版头会先中文确认；当前只支持单机关发文。
