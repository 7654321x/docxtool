...
def handle_ban(handler: Any, parsed: Any, *, is_ip, ban_ip, logger) -> None:
    """传入 handler、已解析 URL、IP 校验和封禁回调，执行封禁并回到安全详情。"""
    ...
    handler._redirect("/admin/web/security?" + urlencode({"ip": ip}))


def handle_unban(handler: Any, parsed: Any, *, is_ip, unban_ip, logger) -> None:
    """传入 handler、已解析 URL、IP 校验和解封回调，执行解封并回到安全详情。"""
    params = handler._request_params(parsed)
    ip = ip_from_action_params(params)
    if not is_ip(ip):
        handler._json_error("INVALID_IP", "无效的 IP", 400)
...
def handle_limit(
    ...
    *,
    ...
    """传入 handler、已解析 URL、默认限额和保存回调，更新上传限额并回到运行设置。"""
    params = handler._request_params(parsed)
    ...
    handler._redirect("/admin/web/runtime")


def handle_cleanup(handler: Any, *, logger) -> None:
    """传入 handler 和日志对象，执行永久保留策略下的兼容清理入口并回到运行设置。"""
