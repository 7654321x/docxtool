...
def render_wps_tasks_page(*, result: Mapping[str, object], filters: Mapping[str, object], csrf_input: str) -> str:
    ...
    for row in result.get("rows", []):
        label, class_name = _status(row.get("status"))
        rows.append(
            f'<tr><td>{_time(row.get("requested_at"))}</td><td>{_escape(row.get("request_id", "-"))}</td><td>{_escape(row.get("username", "-"))}</td><td>{_escape(row.get("device_name", "-"))}</td><td>{_escape(row.get("command", "-"))}</td><td><span class="status-tag {class_name}">{label}</span></td><td>{_escape(row.get("config_version", "-"))}</td><td>{int(row.get("duration_ms", 0) or 0)} ms</td><td>{_escape(row.get("error_code", "-") or "-")}</td><td>{_escape(row.get("app_version", "-") or "-")}</td></tr>'
        )
    body = f"""<section class="panel"><div class="panel-head"><div><h2>WPS 排版任务</h2><p>服务端分页显示授权、成功、失败和待回报请求</p></div>{_filters('/admin/wps/tasks', filters, version=True, request_status=True)}</div><div class="table-wrap"><table><thead><tr><th>请求时间</th><th>请求编号</th><th>账号</th><th>设备</th><th>命令</th><th>状态</th><th>配置</th><th>耗时</th><th>错误码</th><th>版本</th></tr></thead><tbody>{''.join(rows) or '<tr><td colspan="10"><div class="empty">暂无 WPS 排版请求</div></td></tr>'}</tbody></table></div>{_pager('/admin/wps/tasks', result, filters)}</section>"""
