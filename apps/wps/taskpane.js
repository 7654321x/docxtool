(function () {
  ...
  const SAFE_DETAIL_FIELDS = new Set([
    ...
    "direction", "document_scroll_height", "document_scroll_width", "end_offset", "error_code", "error_type", "event_sequence", "header_bottom",
    ...
    "pane_instance_id", "paragraph_count", "pending_present", "previous_status", "readback_present", "reason", "reader_client_height",
    ...
    "reader_scroll_height", "reader_scroll_top", "rendered_paragraph_count", "requested_start_offset", "scheduled_delay_ms", "start_offset", "state_revision", "state_wait_in_flight", "state_wait_stopped", "target_in_current_window", "target_offset", "text_chars", "timer_drift_ms", "navigation_mode", "overlap_px", "page_distance_px", "progress_offset", "progress_percent", "scroll_top_before", "scroll_top_after", "line_height_px", "speed", "previous_window_end_offset",
  ...
  async function bridgeApi(path, body, requestId, method) {
  ...
  function setBusinessButtonsDisabled(disabled) {
    ["preview", "apply", "format_settings", "add_letterhead", "clear_preview", "health"].forEach((id) => { node(id).disabled = Boolean(disabled); });
  ...
  node("format_settings").addEventListener("click", () => {
    void openFormatSettings().catch((error) => {
