...
test("Panel action starts one Host bridge when OnAddinLoad was not observed", async () => {
  ...
  assert.match(harness.taskpaneCreateCalls[0][0], /taskpane\.html\?v=24$/);
  assert.equal(harness.values.get(TASKPANE_VERSION_KEY), "24");
...
test("Panel replaces a stale TaskPane page before showing it", () => {
  ...
  assert.match(harness.taskpaneCreateCalls[1][0], /taskpane\.html\?v=24$/);
  assert.equal(harness.values.get(TASKPANE_VERSION_KEY), "24");
