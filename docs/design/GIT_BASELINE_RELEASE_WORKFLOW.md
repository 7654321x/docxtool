...
- 发布脚本只按允许清单在当前本地仓库暂存文件，并拒绝已有暂存内容。
- 先创建可在本地查询和回退的 Git 提交，再通过 SSH 推送到 GitHub。
- 推送前再次确认远端基线未变化，推送后核验远端提交号等于本地提交号。
- 本地 EXE、运行数据、用户文档、密钥和允许清单外文件不进入提交。

## 接口与兼容性

`scripts/publish_to_github.ps1` 保留 `-DryRun`、`-Quick`、`-Verify`、`-Branch` 和 `-CommitMessage` 入口。`-DryRun` 只检查和展示允许范围内的差异，不修改 Git 索引。默认远端改为 SSH 地址。
